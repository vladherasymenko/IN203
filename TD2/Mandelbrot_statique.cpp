...
computeMandelbrotSet(int W, int H, int maxIter, int nbp, int rank)
{
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < int(H / nbp); ++i) {
            computeMandelbrotSetRow(W, H, maxIter, i, pixels_local.data() + W * (int(H / nbp) - i - 1));
        }
        end = std::chrono::system_clock::now();

        MPI_Gather(pixels_local.data(), W * int(H / nbp), MPI_INT, pixels.data(), W * int(H / nbp), MPI_INT, source, MPI_COMM_WORLD);
        ...
        for (int i = 0; i < int(H / nbp); ++i) {
            computeMandelbrotSetRow(W, H, maxIter, i + rank * (int(H / nbp)), pixels_local.data() + W * (int(H / nbp) - i - 1));
        }
        MPI_Gather(pixels_local.data(), W * int(H / nbp), MPI_INT, pixels.data(), 0, MPI_INT, source, MPI_COMM_WORLD);
    }

    std::vector<int> pixels_buf(W * H);
    for (int j = 0; j < nbp; j++) {
        for (int k = 0; k < W * int(H / nbp); k++) {
            pixels_buf[k + W * int(H / nbp) * (nbp - j - 1)] = pixels[k + W * int(H / nbp) * j];
        }
    }
    pixels = pixels_buf;
    return pixels;
